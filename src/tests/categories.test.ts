...
describe('Categories API', () => {
  ...
  describe('GET /api/v1/categories', () => {
    ...
    it('should only return active categories', async () => {
      ...
      await pool.query(
        'INSERT INTO categories (name, slug, is_active) VALUES (?, ?, ?)',
        ['Test Active', 'test-active', true]
      );
      await pool.query(
        'INSERT INTO categories (name, slug, is_active) VALUES (?, ?, ?)',
        ['Test Inactive', 'test-inactive', false]
      );
      ...
      const activeCategories = response.body.filter((cat: any) => cat.is_active === true);
      const inactiveCategories = response.body.filter((cat: any) => cat.is_active === false);
  ...
  describe('GET /api/v1/categories/:slug', () => {
    it('should return a category by slug', async () => {
      ...
      await pool.query(
        ...
        ['Test Category by Slug', 'test-category-by-slug', 'Test description', true]
      );
    ...
    it('should return 404 for inactive category', async () => {
      ...
      await pool.query(
        'INSERT INTO categories (name, slug, is_active) VALUES (?, ?, ?)',
        ['Test Inactive', 'test-inactive-category', false]
      );
